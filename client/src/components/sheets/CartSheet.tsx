            ...
            <SheetContent side="right" className="flex w-full flex-col p-0 sm:max-w-md">
                    ...
                    <div className="p-4">
                        {cartItems.length === 0 ? (
                            <div className="flex min-h-[55vh] flex-col items-center justify-center rounded-2xl border border-dashed p-6 text-center">
                                <h3 className="text-xl font-semibold">Your cart&apos;s empty</h3>

                                <p className="mt-1 text-sm text-muted-foreground">Add some treats to get started!</p>
                            ...
                            <div className="space-y-3">
                                {cartItems.map((item) => (
                                    <CartItemCard key={item.product.productId} {...item} />
                                ))}
                            </div>
                        )}
                    </div>
                ...
                <SheetFooter className="border-t bg-background p-4">
                    <div className="w-full space-y-2">
                        ...
                        <Button type="button" className="mt-4 w-full" onClick={() => router.push("/checkout")} disabled={cartItems.length === 0}>
